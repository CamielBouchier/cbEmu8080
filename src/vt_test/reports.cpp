      ...
      if (value == 12) {
        if ((value = scan_DA(cmp, &reportpos)) != 0) {
          ...
          case 7:
            ...
            break;
          default:
            cb_printf("unknown code %d", value);
            println("");
            break;
          }
        }
...
tst_DA_2(const char*)
{
  ...
  if ((report = skip_csi(report)) != 0) {
    if (sscanf(report, ">%d;%d;%d%c", &Pp, &Pv, &Pc, &ch) == 4
      ...
      cup(4, 10);
      cb_printf("Pp=%d (%s)", Pp, name);

      cup(5, 10);
      cb_printf("Pv=%d, firmware version %d.%d", Pv, Pv / 10, Pv % 10);

      cup(6, 10);
      switch (Pp) {
      case 64:
      case 65:
        cb_printf("Pc=%d, %s keyboard", Pc, Pc ? "PC" : "VT");
        break;
      default:
        cb_printf("Pc=%d, ROM cartridge registration number", Pc);
        break;
      }
    }
  }
...
tst_DSR(const char*)
{
  ...
  cup(1, 1);
  cb_printf("Test of Device Status Report 5 (report terminal status).");
...
tst_reports(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cb_printf("Terminal Reports/Responses"));
