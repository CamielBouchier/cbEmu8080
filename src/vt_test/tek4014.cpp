...
static int
tek_hello(const char* the_title)
{
  int n;
 ...
 */
static int
tek_mouse_coords(const char* the_title)
{
  char *report;
  char status[6];
  int new_x = -1;
  ...
  do {
     ...
     */
    tek_GIN();
    log_mouse_click(report = InString());
    ...
    if ((report[0] & 0x80) != 0
      ...
      cb_printf("mouse %c:", report[0] & 0x7f);
    } else {
      cb_printf("key: %d", CharOf(report[0]));
    }
    cb_printf(" (%d,%d)", new_y, new_x);
 ...
 */
static int
tek_mouse_lines(const char* the_title)
{
  char *report;
  char status[6];
  int old_x = -1;
 ...
 */
static int
tek_grid_demo(const char* the_title)
{
  int y, x;
  int style = 0;
