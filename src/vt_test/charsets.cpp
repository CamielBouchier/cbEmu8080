...
static int
lookup_Gx(const char* the_title)
{
  int n;
  the_code = -1;
  for (n = 0; n < TABLESIZE(KnownCharsets); n++) {
...
static void
specify_any_Gx(const char* the_title, int g)
{
...
static int
specify_G0(const char* the_title)
{
  specify_any_Gx(the_title, 0);
  return MENU_NOHOLD;
}

static int
specify_G1(const char* the_title)
{
  specify_any_Gx(the_title, 1);
  return MENU_NOHOLD;
}

static int
specify_G2(const char* the_title)
{
  specify_any_Gx(the_title, 2);
  return MENU_NOHOLD;
}

static int
specify_G3(const char* the_title)
{
  specify_any_Gx(the_title, 3);
  return MENU_NOHOLD;
}

static int
tst_layout(const char*)
{
  char buffer[80];
...
tst_vt100_charsets(const char*)
{
  ...
  __(cup(1, 10), cb_printf("Selected as G0 (with SI)"));
  __(cup(1, 48), cb_printf("Selected as G1 (with SO)"));
  ...
  scs_normal();
  __(cup(max_lines, 1), cb_printf("These are the installed character sets. "));
  return MENU_HOLD;
}

static int
tst_shift_in_out(const char*)
{
  /* Test of:
     SCS    (Select character Set)
   */
  ...
  __(cup(1, 10), cb_printf("These are the G0 and G1 character sets."));
...
int
tst_characters(const char* the_title)
{
  ...
  cleanup = 0;
  reset_charset(the_title);   /* make the menu consistent */
  if (get_level() > 1 || input_8bits || output_8bits) {
    do {
      ed(2);
      __(title(0), cb_printf("Character-Set Tests"));
    ...
    cleanup = 1;
    return reset_charset(the_title);
  } else {
    return tst_vt100_charsets(the_title);
  }
}
