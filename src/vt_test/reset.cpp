...
tst_DECTST(const char* the_title)
{
  ...
  if (did_reset)
    ...
    cb_printf("The built-in confidence test ");
  cb_printf("will be invoked. ");
  holdit();
...
tst_RIS(const char* the_title)
{
  ...
  println("");
  cb_printf("The terminal will now be RESET. ");
  holdit();
  ris();
...
tst_rst(const char* the_title)
{
  ...
  do {
    ed(2);
    __(title(0), cb_printf("%s", the_title));
