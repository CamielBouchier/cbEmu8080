...
setup_printout(const char* the_title, int visible, const char *whole)
{
  ...
  cb_printf("lines %d and %d, and write a test pattern there.\r\n", margin_lo, margin_hi);
  cb_printf("The test pattern should be %s.\r\n", visible
         ? "visible"
         : "invisible");
  cb_printf("The %s should be in the printer's output.\r\n", whole);
...
tst_printing(const char*)
{
  ...
  do {
    ...
    ed(2);
    __(title(0), cb_printf("Printing-Control Tests"));
