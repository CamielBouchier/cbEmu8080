...
check_rc(int row, int col)
{
  ...
  if ((params = skip_csi(report)) == 0
    ...
    cb_printf("cursor save/restore %s, got \"%s\", expected \"%s\"",
           SHOW_FAILURE, params, expected);
  } else {
    cb_printf("cursor save/restore %s", SHOW_SUCCESS);
  }
}

static int
test_altscrn_47(const char* the_title)
{
  cup(1, 1);
  println(the_title);
  cup(3, 1);
...
static int
test_altscrn_1047(const char* the_title)
{
  cup(1, 1);
  println(the_title);
  cup(3, 1);
  ...
  cup(7, 5);
  decsc();
  cup(9, 7);  /* move away from the place we saved with DECSC */
...
static int
test_altscrn_1049(const char* the_title)
{
  cup(1, 1);
  println(the_title);
  cup(3, 1);
...
static int
tst_xterm_DECRPM(const char* the_title)
{
  /* *INDENT-OFF* */
  ...
  int code = any_RQM(the_title, dec_modes, TABLESIZE(dec_modes), 1);
