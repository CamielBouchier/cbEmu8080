...
static const int  marNone   = -1;
static const int  marReset  =  0;
static const int  marFirst  =  1;
static const int  marLast   =  2;
static const int  marMiddle =  3;
static const int  marEnd    =  4;
...
static int lr_marg_flag;
static int lr_marg1, lr_marg2;
static int tb_marg_flag;
...
fill_margins(void)
{
  ...
  for (row = top; row <= bot; ++row) {
    ...
    for (col = lft; col <= rgt; ++col) {
      if (row == top) {
        cb_putchar((col - lft) % 10 + '0');
      } else if (row == bot) {
        cb_putchar((col - lft) % 26 + 'a');
      } else {
        cb_putchar('*');
      }
    }
...
static int
show_DECLRMM(const char* the_title)
{
  /* *INDENT-OFF* */
  ...
  code = any_RQM(the_title, dec_modes, TABLESIZE(dec_modes), 1);
...
static int
rpt_DECSACE(const char* the_title)
{
  return any_decrqss(the_title, "*x");
}

static int
rpt_DECSNLS(const char* the_title)
{
  return any_decrqss(the_title, "*|");
}

static int
rpt_DECSLRM(const char* the_title)
{
  return any_decrqss(the_title, "s");
}

static int
rpt_DECELF(const char* the_title)
{
 ...
 */
static int
rpt_DECLFKC(const char* the_title)
{
  return any_decrqss(the_title, "*}");
}

static int
rpt_DECSMKR(const char* the_title)
{
...
show_keypress(int row, int col)
{
  ...
  cup(row, col);
  while (strcmp(report = InString(), last)) {
    cup(row, col);
    ed(0);
    chrprint(report);
    strcpy(last, report);
 ...
 */
static int
tst_DECBI(const char* the_title)
{
  int n, m;
  ...
  cb_printf("should be numbered 1 through %d.\r\n", final);
  return MENU_HOLD;
}

static int
tst_DECBKM(const char* the_title)
{
  char *report;
 ...
 */
static int
tst_DECCARA(const char* the_title)
{
  int last = max_lines - 4;
  BOX box;
...
static int
tst_DECCKSR(const char* the_title, int Pid, const char *the_csi)
{
  char *report;
  int pos = 0;

  cup(1, 1);
  cb_printf("Testing DECCKSR: %s\r\n", the_title);
 ...
 */
static int
tst_DECCRA(const char* the_title)
{
#define adj_y 3
#define adj_x 4
...
tst_DECDC(const char*)
{
  ...
  for (n = 1; n < last_row; n++) {
    ...
    if (row <= last_row) {
      ...
      slowly();
      __(cup(row, col), cb_putchar(mark));
 ...
 */
static int
tst_DECERA(const char* the_title)
{
  int last = max_lines - 3;
  BOX box;
 ...
 */
static int
tst_ASCII_format(const char* the_title)
{
  int last = max_lines - 4;
  int top;
  int bot;
 ...
 */
static int
tst_DECFI(const char* the_title)
{
  int n, m;
  int last = max_lines - 4;
  int final;
  ...
  for (n = 1; n <= final; n++) {
    slowly();
    cup(top, rgt - 3);
    cb_printf("%3d", n);   /* leaves cursor in rightmost column */
    if (n != final) {
      for (m = 0; m < 4; m++)
        decfi();
    }
 ...
 */
static int
tst_cursor_margins(const char* the_title)
{
  BOX box;
 ...
 */
static int
tst_other_margins(const char* the_title)
{
  BOX box;
  int last = get_hold_row();
  int row;
 ...
 */
static int
tst_DECFRA(const char* the_title)
{
  int last = max_lines - 3;
  BOX box;
  ...
  if (do_colors) {
    set_colors(WHITE_ON_BLUE);
    ed(2);  /* xterm fills the whole screen's background */
    set_colors(WHITE_ON_GREEN);
  }
...
static int
tst_DECIC_DECDC(const char* the_title)
{
  tst_DECIC(the_title);
  holdit();
  ed(2);
...
static int
tst_DECKBUM(const char* the_title)
{
  cup(1, 1);
  println(the_title);
...
static int
tst_DECKPM(const char* the_title)
{
  cup(1, 1);
  println(the_title);
...
static int
tst_DECNKM(const char* the_title)
{
  cup(1, 1);
  println(the_title);
 ...
 */
static int
tst_DECRARA(const char* the_title)
{
  int last = max_lines - 4;
  BOX box;
...
tst_vt420_DECRQSS(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cb_printf("VT420 Status-Strings Reports"));
 ...
 */
static int
tst_DECSERA(const char* the_title)
{
  int last = max_lines - 3;
  BOX box;
...
static int
tst_DSR_area_sum(const char* the_title)
{
  /* compute a checksum on the title line, which contains some text */
  return tst_DECCKSR(the_title, 1, "1;1;1;1;2;80*y");
}

static int
tst_DSR_data_ok(const char* the_title)
{
  return any_DSR(the_title, "?75n", show_DataIntegrity);
}

static int
tst_DSR_macrospace(const char* the_title)
{
  char *report;
  const char *show;

  cup(1, 1);
  cb_printf("Testing DECMSR: %s\r\n", the_title);
...
static int
tst_DSR_memory_sum(const char* the_title)
{
  return tst_DECCKSR(the_title, 1, "?63;1n");
}

static int
tst_DSR_multisession(const char* the_title)
{
  return any_DSR(the_title, "?85n", show_MultisessionStatus);
}

int
tst_SRM(const char* the_title)
{
  int oldc, newc;
...
void
setup_vt420_cursor(const char* the_title)
{
  tb_marg_flag = marNone;
  toggle_STBM(the_title);

  lr_marg_flag = marNone;
  toggle_SLRM(the_title);
}
...
void
finish_vt420_cursor(const char* the_title)
{
  reset_colors();
  do_colors = false;
 ...
 */
int
tst_vt420_cursor(const char* the_title)
{
  /* *INDENT-OFF* */
  static MENU my_menu[] = {
      { "Exit",                                              0 },
  ...
  do {
    ed(2);
    __(title(0), cb_printf("VT420 Cursor-Movement Tests"));
    __(title(2), println("Choose test type:"));
    menus_vt420_cursor();
  } while (menu(my_menu));
...
static int
show_DECSTBM(const char* the_title)
{
  int code;
  decstbm(tb_marg1, tb_marg2);
  code = rpt_DECSTBM(the_title);
  return code;
}

static int
show_DECSLRM(const char* the_title)
{
  int code;
  decslrm(lr_marg1, lr_marg2);
  code = rpt_DECSLRM(the_title);
  return code;
}
 ...
 */
static int
tst_VT420_editing(const char* the_title)
{
  /* *INDENT-OFF* */
  static MENU my_menu[] = {
      { "Exit",                                              0 },
  ...
  do {
    ed(2);
    __(title(0), cb_printf("VT420 Editing Sequence Tests"));
    __(title(2), println("Choose test type:"));
    menus_vt420_cursor();
  } while (menu(my_menu));
 ...
 */
static int
tst_VT420_keyboard_ctl(const char*)
{
  /* *INDENT-OFF* */
  static MENU my_menu[] = {
      { "Exit",                                              0 },
  ...
  do {
    ed(2);
    __(title(0), cb_printf("VT420 Keyboard-Control Tests"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  return MENU_NOHOLD;
}
...
static int
tst_VT420_rectangle(const char* the_title)
{
  ...
  do {
    ed(2);
    __(title(0), cb_printf("VT420 Rectangular Area Tests%s",
                        ((terminal_id() < 400)
                         ? " (should not work)"
                         : "")));
    __(title(2), println("Choose test type:"));
...
int
tst_vt420_device_status(const char*)
{
  /* *INDENT-OFF* */
  static MENU my_menu[] = {
      { "Exit",                                              0 },
  ...
  do {
    ed(2);
    __(title(0), cb_printf("VT420 Device Status Reports (DSR)"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  return MENU_NOHOLD;
}
...
int
tst_vt420_report_presentation(const char*)
{
  /* *INDENT-OFF* */
  static MENU my_menu[] = {
      { "Exit",                                              0 },
  ...
  do {
    ed(2);
    __(title(0), cb_printf("VT420 Presentation State Reports"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  set_DECRPM(old_DECRPM);
  return MENU_NOHOLD;
...
tst_vt420_reports(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cb_printf("VT420 Reports"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  return MENU_NOHOLD;
}
...
tst_VT420_screen(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cb_printf("VT420 Screen-Display Tests"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  return MENU_NOHOLD;
}
...
tst_vt420(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cb_printf("VT420 Tests"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  return MENU_NOHOLD;
}
