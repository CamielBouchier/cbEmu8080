...
tst_vt52(const char*)
{
  ...
  vt52cup(7, 47);
  cb_printf("nothing more.");
  for (i = 1; i <= 10; i++)
    cb_printf("THIS SHOULD GO AWAY! ");
  for (i = 1; i <= 5; i++) {
    vt52cup(1, 1);
    cb_printf("%s", "Back scroll (this should go away)");
  ...
  vt52cup(10, 16);
  cb_printf("%s", "The screen should be cleared, and have a centered");
  vt52cup(11, 16);
  cb_printf("%s", "rectangle of \"*\"s with \"!\"s on the inside to the");
  vt52cup(12, 16);
  cb_printf("%s", "left and right. Only this, and");
  vt52cup(13, 16);
  holdit();

  vt52home();   /* Cursor home     */
  vt52ed();     /* Erase to end of screen  */
  cb_printf("%s", "This is the normal character set:");
  for (j = 0; j <= 1; j++) {
    vt52cup(3 + j, 16);
    for (i = 0; i <= 47; i++)
      cb_printf("%c", 32 + i + 48 * j);
  }
  vt52cup(6, 1);
  cb_printf("%s", "This is the special graphics character set:");
  ...
  if (terminal_id() >= 200) {
    ...
    if (save.cur_level >= 2) {
      ...
      response = InString();
      cup(row, 10);
      cb_printf("Response to CUP(1,1)/DSR(6)");
      chrprint(response);
      if ((temp = skip_prefix(csi_input(), response)) != 0) {
        if (!strcmp("1;1R", temp)) {
          cb_printf("S8C1T recognized --");
          show_result(SHOW_FAILURE);
        } else {
          cb_printf("unknown response --");
          show_result(SHOW_FAILURE);
        }
      } else {
