...
check_box_params(BOX *box)
{
  if (box->top >= box->bottom || box->left >= box->right) {
    cb_printf("The screen is too small for box with margins %d,%d.", box->top, box->left);
    holdit();
    return -1;
  }
  return 0;
...
draw_box_outline(BOX *box, int mark)
{
  ...
  for (j = box->top, dot = tlc; j < box->bottom; j++) {
    __(cup(j, box->left), cb_putchar(dot));
    dot = vrt;
  }
  for (j = box->top, dot = trc; j < box->bottom; j++) {
    __(cup(j, box->right), cb_putchar(dot));
    dot = vrt;
  }
  ...
  __(cup(box->bottom, box->left), cb_putchar(llc));
  __(cup(box->bottom, box->right), cb_putchar(lrc));
