...
{
  /* *INDENT-OFF* */
  cup(first, 1);          cb_printf("+");
  cup(last-1,  1);        cb_printf("+");
  cup(first, min_cols);   cb_printf("+");
  cup(last-1,  min_cols); cb_printf("+");
  /* *INDENT-ON* */
}
...
static void
show_dec_locator_events(const char* the_title, int mode, int pixels)
{
  int row, now;
...
static void
show_mouse_tracking(const char* the_title, const char *the_mode)
{
  unsigned y = 0, x = 0;
  unsigned b, xx, yy;
...
static int
test_dec_locator_event(const char* the_title)
{
  show_dec_locator_events(the_title, 2, false);
  return MENU_HOLD;
}

static int
test_dec_locator_events(const char* the_title)
{
  show_dec_locator_events(the_title, 1, false);
  return MENU_HOLD;
}

static int
test_dec_locator_event_p(const char* the_title)
{
  show_dec_locator_events(the_title, 2, true);
  return MENU_HOLD;
}

static int
test_dec_locator_events_p(const char* the_title)
{
  show_dec_locator_events(the_title, 1, true);
  return MENU_HOLD;
}

static int
test_dec_locator_rectangle(const char* the_title)
{
  show_dec_locator_events(the_title, -2, false);
  return MENU_HOLD;
}
...
static int
test_dec_locator_unfiltered(const char* the_title)
{
  show_dec_locator_events(the_title, 0, false);
  return MENU_HOLD;
}

/* Any-Event Mouse Tracking */
static int
test_mouse_any_event(const char* the_title)
{
  show_mouse_tracking(the_title, "?1003");
  return MENU_HOLD;
}

/* Button-Event Mouse Tracking */
static int
test_mouse_button_event(const char* the_title)
{
  show_mouse_tracking(the_title, "?1002");
  return MENU_HOLD;
}

/* Mouse Highlight Tracking */
static int
test_mouse_hilite(const char* the_title)
{
  const int first = 10;
  const int last = 20;
  unsigned b;
...
first:
  ...
  show_mousehelp();
  println("Mouse events will be marked with the button number.");
  cb_printf("Highlighting range is [%d..%d)\r\n", first, last);
  show_hilite(first, last);
...
/* Normal Mouse Tracking */
static int
test_mouse_normal(const char* the_title)
{
  show_mouse_tracking(the_title, "?1000");
  return MENU_HOLD;
}

/* X10 Mouse Compatibility */
static int
test_X10_mouse(const char* the_title)
{
  unsigned b, x, y;
 ...
 */
static int
tst_dec_locator_events(const char*)
{
