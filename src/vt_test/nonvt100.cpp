...
int
not_impl(const char* the_title)
{
  cup(1, 1);
  cb_printf("Sorry, test not implemented:\r\n\r\n  %s", the_title);
  cup(max_lines - 1, 1);
  return MENU_HOLD;
}
  ...
  if (make_box_params(&box, 5, 20) == 0) {
    for (pass = 0; pass < 2; pass++) {
      ...
      if (pass == 0) {
        ...
        cup(max_lines - 4, 1);
        cb_printf("There %s be an solid box made of *'s in the middle of the screen.\r\n",
               erm_flag
               ? "may"
               : "should");
...
static int
tst_protected_area(const char* the_title)
{
  ...
  do {
    ed(2);
    __(title(0), cb_printf("Protected-Areas Tests"));
    __(title(2), println("Choose test type:"));
    sprintf(erm_mesg, "%s ERM (erase mode)", STR_ENABLE(erm_flag));
  } while (menu(my_menu));
  if (erm_flag)
    toggle_ERM(the_title);
  return MENU_NOHOLD;
}
...
tst_ecma48_curs(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cb_printf("ISO-6429 (ECMA-48) Cursor-Movement"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  return MENU_NOHOLD;
}
...
tst_ecma48_misc(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cb_printf("Miscellaneous ISO-6429 (ECMA-48) Tests"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  return MENU_NOHOLD;
}
