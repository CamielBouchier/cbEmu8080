...
int
any_DSR(const char* the_title, const char *text, void (*explain) (char *report))
{
  char *report;
  unsigned pmode = (unsigned) ((*text == '?') ? 1 : 0);

  cup(1, 1);
  cb_printf("Testing DSR: %s\r\n", the_title);
 ...
 */
static int
tst_S8C1T(const char* the_title) {
  char *report;
  ...
  for (pass = 0; pass < 2; pass++) {
    ...
    report = InString();
    cup(10 + pass * 3, 1);
    cb_printf("8-bit controls %s: ", STR_ENABLED(flag));
    chrprint(report);
...
static int
tst_DECUDK(const char* the_title)
{
  int key;
  /* *INDENT-OFF* */
  static struct {
...
/* vt220 & up */
int
tst_DSR_keyboard(const char* the_title)
{
  return any_DSR(the_title, "?26n", show_KeyboardStatus);
}

int
tst_DSR_printer(const char* the_title)
{
  return any_DSR(the_title, "?15n", show_PrinterStatus);
}

int
tst_DSR_userkeys(const char* the_title)
{
  return any_DSR(the_title, "?25n", show_UDK_Status);
}
...
static int
tst_DSR_operating_status(const char* the_title)
{
  return any_DSR(the_title, "5n", show_OperatingStatus);
}
...
tst_ECH(const char*)
{
  ...
  for (i = 1; i <= max_lines; i++) {
    ...
    cb_printf("*");
    ech(min_cols);
    cb_printf("*");  /* this should be adjacent, in the upper-right corner */
  }

  cup(last, 1);
  ed(0);

  cup(last, min_cols - (last + 10));
...
int
tst_vt220_device_status(const char*)
{
  /* *INDENT-OFF* */
  static MENU my_menu[] = {
      { "Exit",                                              0 },
  ...
  do {
    ed(2);
    title(0);
    cb_printf("VT220 Device Status Reports");
    title(2);
...
tst_vt220_screen(const char*)
{
  ...
  do {
    ed(2);
    title(0);
    cb_printf("VT220 Screen-Display Tests");
    title(2);
    println("Choose test type:");
  } while (menu(my_menu));
  return MENU_NOHOLD;
...
tst_vt220_reports(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cb_printf("VT220 Reports"));
