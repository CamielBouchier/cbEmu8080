...
tst_DECDLD(const char*)
{
  ...
  cb_printf("%c*%s", ESC, FontName);   /* designate G2 as the DRCS font */
...
tst_display(const char*)
{
  ...
  do {
    ...
    if (display_char(c)) {
      println("");
      cb_printf("Render: %cN%c", ESC, c);  /* use SS2 to invoke G2 into GL */
    }
  } while (c != d);
...
tst_softchars(const char*)
{
  ...
  if (font_string == 0 || *font_string == 0) {
    cb_printf("You did not specify a font-file with the -f option\r\n");
    return MENU_HOLD;
  }
  do {
    ed(2);
    __(title(0), cb_printf("Soft Character Sets"));
