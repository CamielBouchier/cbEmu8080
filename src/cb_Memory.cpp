...
cb_Memory::cb_Memory(const QString& ImageName) 
    {
    Reset();
    // Putting the signalling back here, ensures the initial load
    // - based on user settings - is signalled.
    connect(this, &cb_Memory::SignalImageLoaded, Emu8080, &cb_emu_8080::OnMemoryImageLoaded);
    if (ImageName.size())
        {
...
void cb_Memory::SaveToFile(const QString& FileName) 
    {
...
void cb_Memory::LoadFromFile(const QString& FileName) 
    {
