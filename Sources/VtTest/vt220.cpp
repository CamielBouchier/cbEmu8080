...
tst_S8C1T(const char* the_title) {
  ...
  for (pass = 0; pass < 2; pass++) {
    ...
    report = InString();
    cup(10 + pass * 3, 1);
    cbprintf("8-bit controls %s: ", STR_ENABLED(flag));
    chrprint(report);
...
tst_ECH(const char*)
{
  ...
  for (i = 1; i <= max_lines; i++) {
    ...
    cbprintf("*");
    ech(min_cols);
    cbprintf("*");  /* this should be adjacent, in the upper-right corner */
  }
...
tst_vt220_device_status(const char*)
{
  ...
  do {
    ed(2);
    title(0);
    cbprintf("VT220 Device Status Reports");
    title(2);
...
tst_vt220_screen(const char*)
{
  ...
  do {
    ed(2);
    title(0);
    cbprintf("VT220 Screen-Display Tests");
    title(2);
    println("Choose test type:");
  } while (menu(my_menu));
  return MENU_NOHOLD;
...
tst_vt220_reports(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cbprintf("VT220 Reports"));
