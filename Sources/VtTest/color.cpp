...
show_graphic_rendition(void)
{
  ed(2);
  /* *INDENT-OFF* */
  cup( 1,20); cbprintf("Color/Graphic rendition test pattern:");
  cup( 4, 1); c_sgr("0");            cbprintf("vanilla");
  cup( 4,40); c_sgr("0;1");          cbprintf("bold");
  cup( 6, 6); c_sgr(";4");           cbprintf("underline");
  cup( 6,45); c_sgr(";1");c_sgr("4");cbprintf("bold underline");
  cup( 8, 1); c_sgr("0;5");          cbprintf("blink");
  cup( 8,40); c_sgr("0;5;1");        cbprintf("bold blink");
  cup(10, 6); c_sgr("0;4;5");        cbprintf("underline blink");
  cup(10,45); c_sgr("0;1;4;5");      cbprintf("bold underline blink");
  cup(12, 1); c_sgr("1;4;5;0;7");    cbprintf("negative");
  cup(12,40); c_sgr("0;1;7");        cbprintf("bold negative");
  cup(14, 6); c_sgr("0;4;7");        cbprintf("underline negative");
  cup(14,45); c_sgr("0;1;4;7");      cbprintf("bold underline negative");
  cup(16, 1); c_sgr("1;4;;5;7");     cbprintf("blink negative");
  cup(16,40); c_sgr("0;1;5;7");      cbprintf("bold blink negative");
  cup(18, 6); c_sgr("0;4;5;7");      cbprintf("underline blink negative");
  cup(18,45); c_sgr("0;1;4;5;7");    cbprintf("bold underline blink negative");
  cup(20, 6); c_sgr(""); set_foreground(9); cbprintf("original foreground");
  cup(20,45); c_sgr(""); set_background(9); cbprintf("original background");
  /* *INDENT-ON* */
...
show_line_deletions(void)
{
  ...
  ed(2);
  cup(1, 1);
  cbprintf("This test deletes every third line from a list, marking cursor with '*'.\r\n");
  cbprintf("The foreground and background should be yellow(orange) and blue, respectively.\r\n");

  for (row = 5; row <= max_lines; row++) {
    cup(row, 1);
    cbprintf("   row %3d: this is some text", row);
  }
  for (row = 7; row <= max_lines; row += 2 /* 3 - deletion */ ) {
    cup(row, 8);
    dl(1);
    cbputchar('*');/* cursor should be in column 1 XXX CB : NO 8. Cfr xterm */
  }
  cup(3, 1);
  holdit();
}
...
show_line_insertions(void)
{
  ...
  ed(2);
  cup(1, 1);
  cbprintf("This test inserts after every second line in a list, marking cursor with '*'.\r\n");
  cbprintf("The foreground and background should be yellow(orange) and blue, respectively.\r\n");
...
/* generate a color test pattern */
{
  ...
  ed(2);
  cup(1, 1);
  cbprintf("There are %d color combinations", MAX_COLORS * MAX_COLORS);

  for (k = 0; k <= 11; k += 11) {
    cup(k + 2, 1);
    cbprintf("%dx%d matrix of foreground/background colors, bright *",
...
test_iso_6429_sgr(const char*)
{
  ...
  ed(2);
  /* *INDENT-OFF* */
  cup( 1,20); cbprintf("Extended/Graphic rendition test pattern:");
  cup( 4, 1); c_sgr("0");            cbprintf("vanilla");
  cup( 4,40); c_sgr("0;1");          cbprintf("bold");
  cup( 6, 6); c_sgr("22;4");         cbprintf("underline");
  cup( 6,45); c_sgr("24;1;4");       cbprintf("bold underline");
  cup( 8, 1); c_sgr("22;24;5");      cbprintf("blink");
  cup( 8,40); c_sgr("25;5;1");       cbprintf("bold blink");
  cup(10, 6); c_sgr("22;4;5");       cbprintf("underline blink");
  cup(10,45); c_sgr("24;25;1;4;5");  cbprintf("bold underline blink");
  cup(12, 1); c_sgr("22;24;25;7");   cbprintf("negative");
  cup(12,40); c_sgr("1");            cbprintf("bold negative");
  cup(14, 6); c_sgr("22;4;7");       cbprintf("underline negative");
  cup(14,45); c_sgr("1;4;7");        cbprintf("bold underline negative");
  cup(16, 1); c_sgr("22;24;5;7");    cbprintf("blink negative");
  cup(16,40); c_sgr("1");            cbprintf("bold blink negative");
  cup(18, 6); c_sgr("22;4");         cbprintf("underline blink negative");
  cup(18,45); c_sgr("1");            cbprintf("bold underline blink negative");
  cup(20, 6); c_sgr(""); set_foreground(9); cbprintf("original foreground");
  cup(20,45); c_sgr(""); set_background(9); cbprintf("original background");
  cup(22, 1); c_sgr(";8");           cbprintf("concealed");
  cup(22,40); c_sgr("8;7");          cbprintf("concealed negative");
  /* *INDENT-ON* */

  c_sgr("");    /* same as c_sgr("0") */
  cbprintf(" <- concealed text");
...
test_SGR_0(const char* the_title)
{
  ...
  reset_all_colors();
  cbprintf("You should see only black:");
  sgr("30;40");
  cbprintf("SGR 30 and SGR 40 don't work");
  reset_all_colors();
  println(":up to here");

  reset_all_colors();
  cbprintf("You should see only white:");
  sgr("37;47");
  cbprintf("SGR 37 and SGR 47 don't work");
  reset_all_colors();
  println(":up to here");

  reset_all_colors();
  cbprintf("You should see text here: ");
  sgr("30;40");
  sgr("0");
  cbprintf("SGR 0 reset works (explicit 0)");
  println("");
  ...
  sgr("37;47");
  sgr("");
  cbprintf("SGR 0 reset works (default param)");
  println("");
