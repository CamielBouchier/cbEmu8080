...
tst_vt52(const char*)
{
  ...
  vt52cup(7, 47);
  cbprintf("nothing more.");
  for (i = 1; i <= 10; i++)
    cbprintf("THIS SHOULD GO AWAY! ");
  for (i = 1; i <= 5; i++) {
    vt52cup(1, 1);
    cbprintf("%s", "Back scroll (this should go away)");
  ...
  vt52cup(10, 16);
  cbprintf("%s", "The screen should be cleared, and have a centered");
  vt52cup(11, 16);
  cbprintf("%s", "rectangle of \"*\"s with \"!\"s on the inside to the");
  vt52cup(12, 16);
  cbprintf("%s", "left and right. Only this, and");
  vt52cup(13, 16);
  holdit();

  vt52home();   /* Cursor home     */
  vt52ed();     /* Erase to end of screen  */
  cbprintf("%s", "This is the normal character set:");
  for (j = 0; j <= 1; j++) {
    vt52cup(3 + j, 16);
    for (i = 0; i <= 47; i++)
      cbprintf("%c", 32 + i + 48 * j);
  }
  vt52cup(6, 1);
  cbprintf("%s", "This is the special graphics character set:");
  ...
  if (terminal_id() >= 200) {
    ...
    if (save.cur_level >= 2) {
      ...
      response = InString();
      cup(row, 10);
      cbprintf("Response to CUP(1,1)/DSR(6)");
      chrprint(response);
      if ((temp = skip_prefix(csi_input(), response)) != 0) {
        if (!strcmp("1;1R", temp)) {
          cbprintf("S8C1T recognized --");
          show_result(SHOW_FAILURE);
        } else {
          cbprintf("unknown response --");
          show_result(SHOW_FAILURE);
        }
      } else {
