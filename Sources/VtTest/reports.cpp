...
tst_DA_2(const char*)
{
  ...
  if ((report = skip_csi(report)) != 0) {
    if (sscanf(report, ">%d;%d;%d%c", &Pp, &Pv, &Pc, &ch) == 4
      ...
      cup(4, 10);
      cbprintf("Pp=%d (%s)", Pp, name);

      cup(5, 10);
      cbprintf("Pv=%d, firmware version %d.%d", Pv, Pv / 10, Pv % 10);

      cup(6, 10);
      switch (Pp) {
      case 64:
      case 65:
        cbprintf("Pc=%d, %s keyboard", Pc, Pc ? "PC" : "VT");
        break;
      default:
        cbprintf("Pc=%d, ROM cartridge registration number", Pc);
        break;
      }
    }
  }
...
tst_DSR(const char*)
{
  ...
  cup(1, 1);
  cbprintf("Test of Device Status Report 5 (report terminal status).");
...
tst_reports(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cbprintf("Terminal Reports/Responses"));
