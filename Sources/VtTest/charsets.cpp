...
tst_vt100_charsets(const char*)
{
  ...
  __(cup(1, 10), cbprintf("Selected as G0 (with SI)"));
  __(cup(1, 48), cbprintf("Selected as G1 (with SO)"));
  ...
  scs_normal();
  __(cup(max_lines, 1), cbprintf("These are the installed character sets. "));
  return MENU_HOLD;
}
...
tst_shift_in_out(const char*)
{
  ...
  __(cup(1, 10), cbprintf("These are the G0 and G1 character sets."));
...
tst_characters(const char* the_title)
{
  ...
  if (get_level() > 1 || input_8bits || output_8bits) {
    do {
      ed(2);
      __(title(0), cbprintf("Character-Set Tests"));
