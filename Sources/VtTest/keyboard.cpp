...
show_cursor_keys(int flag)
{
  ...
  for (i = 0; curkeytab[i].curkeysymbol[0] != '\0'; i++) {
    ...
    if (flag)
      sgr("7");
    cbprintf("%s", curkeytab[i].curkeysymbol);
    if (flag)
      sgr("");
  }
}
...
show_editing_keypad(int flag)
{
  if (terminal_id() >= 200) {
    ...
    for (i = 0; edt_keypadtab[i].fnkeysymbol[0] != '\0'; i++) {
      ...
      if (flag)
        sgr("7");
      cbprintf("%s", edt_keypadtab[i].fnkeysymbol);
      if (flag)
        sgr("");
    }
  }
...
show_function_keys(int flag)
{
  if (terminal_id() >= 200) {
    ...
    for (i = 0; fnkeytab[i].fnkeysymbol[0] != '\0'; i++) {
      ...
      if (flag)
        sgr("7");
      cbprintf("%s", fnkeytab[i].fnkeysymbol);
      if (flag)
        sgr("");
    }
  }
...
show_numeric_keypad(int flag)
{
  ...
  for (i = 0; num_keypadtab[i].fnkeysymbol[0] != '\0'; i++) {
    ...
    if (flag)
      sgr("7");
    cbprintf("%s", num_keypadtab[i].fnkeysymbol);
    if (flag)
      sgr("");
  }
}
...
tst_ControlKeys(const char*)
{
  ...
  do {
    ...
    el(0);
    if (kbdc < 32) {
      cbprintf("  %s", ckeytab[kbdc].csymbol);
    } else {
      sprintf(kbds, "%c", kbdc);
      chrprint(kbds);
      cbprintf("%s", " -- not a CTRL key");
    }
    if (kbdc < 32)
      ckeytab[kbdc].ccount++;
  ...
  if (okflag)
    ...
    cbprintf("%s", "You have not been able to send all CTRL keys! ");
  return MENU_HOLD;
}
...
tst_CursorKeys(const char*)
{
  ...
  for (ckeymode = 0; ckeymode <= 2; ckeymode++) {
    ...
    show_cursor_keys(1);
    cup(21, 1);
    cbprintf("<%s>%20s", curkeymodes[ckeymode], "");
    cup(max_lines - 2, 1);
    el(0);
    cup(max_lines - 2, 1);
    cbprintf("%s", "Press each cursor key. Finish with TAB.");
    for (;;) {
      cup(max_lines - 1, 1);
      if (ckeymode == 2)
...
tst_EditingKeypad(const char*)
{
  ...
  if (terminal_id() < 200) {
    cbprintf("Sorry, a real VT%d terminal doesn't have an editing keypad\r\n",
           terminal_id());
    return MENU_HOLD;
  } 
  ...
  for (fkeymode = 0; fkeymode <= 1; fkeymode++) {
    ...
    cup(21, 1);
    cbprintf("<%s>%20s", fnkeymodes[fkeymode], "");
    cup(max_lines - 2, 1);
    el(0);
    cup(max_lines - 2, 1);
    cbprintf("%s", "Press each function key. Finish with TAB.");
...
tst_FunctionKeys(const char*)
{
  ...
  if (terminal_id() < 200) {
    cbprintf("Sorry, a real VT%d terminal doesn't have function keys\r\n",
           terminal_id());
    return MENU_HOLD;
  } 
...
tst_keyboard_layout(char *scs_params)
{
  ...
  InFlush();
  cbprintf("Press each key, both shifted and unshifted. Finish with RETURN:");
  do {          /* while (kbdc != 13) */
    cup(max_lines - 1, 1);
    kbdc = InChar();
    cup(max_lines - 1, 1);
...
tst_keyboard(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cbprintf("Keyboard Tests"));
