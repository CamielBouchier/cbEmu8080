...
fill_margins(void)
{
  ...
  for (row = top; row <= bot; ++row) {
    ...
    for (col = lft; col <= rgt; ++col) {
      if (row == top) {
        cbputchar((col - lft) % 10 + '0');
      } else if (row == bot) {
        cbputchar((col - lft) % 26 + 'a');
      } else {
        cbputchar('*');
      }
    }
...
tst_DECBI(const char* the_title)
{
  ...
  cbprintf("should be numbered 1 through %d.\r\n", final);
  return MENU_HOLD;
}
...
tst_DECFI(const char* the_title)
{
  ...
  for (n = 1; n <= final; n++) {
    slowly();
    cup(top, rgt - 3);
    cbprintf("%3d", n);   /* leaves cursor in rightmost column */
    if (n != final) {
      for (m = 0; m < 4; m++)
        decfi();
    }
...
tst_vt420_DECRQSS(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cbprintf("VT420 Status-Strings Reports"));
...
tst_vt420_cursor(const char* the_title)
{
  ...
  do {
    ed(2);
    __(title(0), cbprintf("VT420 Cursor-Movement Tests"));
    __(title(2), println("Choose test type:"));
    menus_vt420_cursor();
  } while (menu(my_menu));
...
tst_VT420_editing(const char* the_title)
{
  ...
  do {
    ed(2);
    __(title(0), cbprintf("VT420 Editing Sequence Tests"));
    __(title(2), println("Choose test type:"));
    menus_vt420_cursor();
  } while (menu(my_menu));
...
tst_VT420_keyboard_ctl(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cbprintf("VT420 Keyboard-Control Tests"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  return MENU_NOHOLD;
}
...
tst_VT420_rectangle(const char* the_title)
{
  ...
  do {
    ed(2);
    __(title(0), cbprintf("VT420 Rectangular Area Tests%s",
                        ((terminal_id() < 400)
                         ? " (should not work)"
                         : "")));
    __(title(2), println("Choose test type:"));
...
tst_vt420_device_status(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cbprintf("VT420 Device Status Reports (DSR)"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  return MENU_NOHOLD;
}
...
tst_vt420_report_presentation(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cbprintf("VT420 Presentation State Reports"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  set_DECRPM(old_DECRPM);
  return MENU_NOHOLD;
...
tst_vt420_reports(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cbprintf("VT420 Reports"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  return MENU_NOHOLD;
}
...
tst_VT420_screen(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cbprintf("VT420 Screen-Display Tests"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  return MENU_NOHOLD;
}
...
tst_vt420(const char*)
{
  ...
  do {
    ed(2);
    __(title(0), cbprintf("VT420 Tests"));
    __(title(2), println("Choose test type:"));
  } while (menu(my_menu));
  return MENU_NOHOLD;
}
