...
QByteArray cbExpNode::EvaluateInt(qint32& Value, bool ReportError)
    {
    QByteArray RV;
    switch (m_Type)
        {
...
QByteArray cbExpNode::EvaluateByteWord(bool        Word, 
                                       QByteArray& Bytes,
                                       cbLine*&    Line,
                                       bool        ReportError)
    {
...
const char* cbExpNode::NameFromType(int Type)
    { 
    switch(Type)
        {  
...
const char* cbExpNode::NameFromTarget(int Target)
    {
    switch(Target)
        {
...
void cbExpNode::Print(FILE* File, int Depth )
    {
    for (int i=0; i<Depth; i++) fprintf(File, "\t");
    fprintf(File, "Target : %s\n", NameFromTarget(m_Target));
    for (int i=0; i<Depth; i++) fprintf(File, "\t");
    if (m_Type == cbExpNode::Type_Identifier)
        {
        fprintf(File, "Identifier : '%s'\n", m_Value.toByteArray().data());
        }
    else if (m_Type == cbExpNode::Type_Integer)
        {
