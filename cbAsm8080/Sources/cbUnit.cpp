...
void cbUnit::ResolveExpressions(const bool ReportError)
    {
    for (auto Line : m_Lines)
        {
        if (not Line->m_HaveValue and 
            Line->m_Expression and 
            Line->m_Expression->m_Target == cbExpNode::Target_Integer)
            {
            int Value;
        ...
        if (not Line->m_HaveValue and Line->m_Expression and 
            ( Line->m_Expression->m_Target == cbExpNode::Target_Bytes or 
              Line->m_Expression->m_Target == cbExpNode::Target_Words) )
            {
            QByteArray ErrMsg = Line->m_Expression->
                EvaluateByteWord(Line->m_Expression->m_Target == cbExpNode::Target_Words,
                                 Line->m_Bytes, 
...
QByteArray cbUnit::AddIdentifier(Identifier* Id)
    {
    QByteArray RV;
...
QByteArray cbUnit::FindIdentifier(QByteArray Id, int&  Value) 
    {
    QByteArray RV;
    for (auto X : m_Identifiers)
        {
...
void cbUnit::DumpIdentifiers(FILE* File)
    {
    for (auto X : m_Identifiers)
        {
        fprintf(File,
...
void cbUnit::CheckRange(cbLine* Line, bool Word)
    {
    bool Fit = true;
...
void cbUnit::DumpList(FILE* File)
    {
    for (auto Line : m_Lines)
        {
...
void cbUnit::ErrorMessage(cbLine* Line, const char* Format, ...)
    {
    GlobalError = true;
